...
{#if $modal.component}
  <div
    ...
    class="fixed inset-0 {$settings.invertColors
      ? 'bg-white'
      : 'bg-black'} bg-opacity-50 firefox:bg-opacity-50 filter drop-shadow-xl backdrop-filter firefox:backdrop-filter backdrop-blur firefox:backdrop-blur z-20 flex justify-center items-center"
    transition:fade
